...
<section>
	...
	<div class="left">
        <ul>
            ...
            <li class="active2"><a href="message.php" style="position: relative;">Messages</a>
            </li>
        </ul>
	</div>
        ...
        <div class="personal_des ">
            ...
            <div class="inforcon">
                ...
                <form class="perinform" id="jsEditUserForm" name="info_form" autocomplete="off">
                    ...
                    <?php
                    $charge_query = "SELECT * FROM Record where login_name = '$user_name' order by pledgetime desc";
                    $charge = mysqli_query($dbc,$charge_query);
                    if(mysqli_num_rows($charge)>0){
                        while($rows=mysqli_fetch_assoc($charge)){
                                    

                                    $proid = $rows["proid"];

                                    $proname_query = "SELECT * FROM Projects WHERE proid = $proid";
                                    $var_proname = mysqli_fetch_array(mysqli_query($dbc,$proname_query));
                                    $proname = $var_proname["proname"];

                                    $creditcard = $rows["creditcard"];
                                    
                                    $pledgetime = $rows["pledgetime"];
                                    
                                    $chargetime = $rows["chargetime"];
